(function () {
  ...
  function dropTable(s) {
    ...
    return `<div class="statrow"><div class="stat"><b>${s.kills}</b>total runs</div><div class="stat"><b>${uniqueRate(s)}</b>uniques dropped</div><div class="stat"><b>${infreqRate(s)}</b>infrequents dropped</div><div class="stat"><b>${s.gold || "\u2014"}</b>avg gold / band</div><div class="stat"><b>${s.namedEssences ?? "\u2014"}</b>named essences</div></div>${s.notes ? `<p class="lede">${s.notes}</p>` : ""}${s.drops && s.drops.length ? `<table class="drops"><thead><tr><th>Item</th><th>n</th><th>Observed</th></tr></thead><tbody>${s.drops.map((d) => `<tr><td>${itemLink(d.item)}${d.flag ? ' <span class="pill r-off">'+d.flag+'</span>' : ""}</td><td>${d.count}</td><td>${rate(d.count, s.kills)}</td></tr>`).join("")}</tbody></table>` : ""}${s.infrequentNames ? `<h3>Infrequent names seen</h3><ul>${s.infrequentNames.map((n)=>`<li>${n}</li>`).join("")}</ul>` : ""}`;
  }
  function statusOf(b) {
    const s = sampleOf(b);
  ...
  function bossCard(b) {
    const s = sampleOf(b);
    const st = statusOf(b);
    return `<a class="card" href="#/boss/${b.id}"><div class="card-top"><span class="pill act">Act ${b.act}</span><span class="pill r-${st === "closed" ? "named" : st === "open" ? "infreq" : st === "verify" ? "off" : "gold"}">${st}</span></div><h3>${b.name}</h3><div class="meta">${b.title || ""}</div><div class="meta">${s ? (s.label + " \u00b7 " + s.kills + " kills") : (b.remnantConfirmed ? "remnant confirmed" : "remnant unconfirmed")}</div></a>`;
  }
  function bossesByAct() {
    const acts = {};
    D.bosses.forEach((b) => { const key = b.actName || ("Act " + b.act); (acts[key] = acts[key] || []).push(b); });
    return acts;
  }
  function home() {
    ...
    return `<h1>Titan Quest 2 database</h1><p class="lede">Observed remnant drop tables. Cap ${D.meta.cap}, ${D.meta.chapter}, updated ${D.meta.updated}.</p><div class="statrow"><div class="stat"><b>${D.items.length}</b>catalogued items</div><div class="stat"><b>${D.bosses.length}</b>boss stubs</div><div class="stat"><b>${closed.reduce((a,s)=>a+s.kills,0)}</b>closed-sample kills</div><div class="stat"><b>${open.reduce((a,s)=>a+s.kills,0)}</b>open-sample kills</div></div>${Object.entries(acts).map(([name, list]) => `<h2>${name}</h2><div class="grid">${list.map(bossCard).join("")}</div>`).join("")}`;
  }
  function isBase(i) { return !i.aliasOf && (i.unique || i.rarity === "epic" || i.rarity === "infrequent"); }
  function itemsView(filter) {
    ...
    return `<h1>Items</h1><p class="lede">Bases only. Rolled infrequent names live on the base page.</p><div class="filters">${["all","unique","infrequent","named-essence"].map((f) => `<a href="#/items/${f}"><button class="${(!filter || filter==="all") && f==="all" || filter===f ? "on":""}">${f}</button></a>`).join("")}</div><table><thead><tr><th>Name</th><th>Type</th><th>Slot</th><th>Variants seen</th></tr></thead><tbody>${list.map((i) => `<tr><td>${itemLink(i.id)}</td><td><span class="pill r-${i.rarity.startsWith("epic")||i.unique?"epic":i.rarity}">${i.unique?"unique":i.rarity}</span></td><td>${i.slot||""}</td><td class="meta">${(i.variants&&i.variants.length)?(i.variants.length+" logged"):(i.unique?"unique base":"")}</td></tr>`).join("")}</tbody></table>`;
  }
  function itemView(id) {
    ...
    const sources = [];
    D.bosses.forEach((b) => (b.samples || []).forEach((s) => { (s.drops || []).forEach((d) => { if (d.item === i.id || (itemById[d.item] && itemById[d.item].aliasOf === i.id)) sources.push({ boss: b, sample: s, drop: d }); }); }));
    return `<p class="meta"><a href="#/items">Items</a></p><h1 class="item-name ${i.rarity}">${i.name}</h1><p><span class="pill r-${i.rarity.startsWith("epic")?"epic":i.rarity}">${i.rarity}</span> ${i.slot||""} ${i.unique?"\u00b7 unique":""}</p>${i.notes?`<p class="lede">${i.notes}</p>`:""}${i.mods?`<ul class="mods">${i.mods.map((m)=>`<li>${m}</li>`).join("")}</ul>`:""}${i.variants&&i.variants.length?`<h2>Found variants</h2><table><thead><tr><th>Rolled name</th><th>Seen</th><th>Flag</th></tr></thead><tbody>${i.variants.map((v)=>`<tr><td>${v.name}</td><td>${v.seen||""}</td><td>${v.flag||""}</td></tr>`).join("")}</tbody></table>`:""}<h2>Observed sources</h2>${sources.length?`<table><thead><tr><th>Boss</th><th>Sample</th><th>Rate</th></tr></thead><tbody>${sources.map((s)=>`<tr><td><a href="#/boss/${s.boss.id}">${s.boss.name}</a></td><td>${s.sample.label} (${s.sample.kills})</td><td>${rate(s.drop.count,s.sample.kills)}</td></tr>`).join("")}</tbody></table>`:"<p class='meta'>No remnant sample attached yet.</p>"}`;
  }
  function dropsView() {
    const acts = bossesByAct();
    return `<h1>Drop tables</h1>${Object.entries(acts).map(([name, list]) => `<h2>${name}</h2><div class="grid">${list.map(bossCard).join("")}</div>`).join("")}`;
  }
  function bossView(id) {
    const b = D.bosses.find((x)=>x.id===id);
    if (!b) return "<p>Unknown boss.</p>";
    const s0 = sampleByAres(b, 0); const s3 = sampleByAres(b, 3); const s5 = sampleByAres(b, 5);
    const portrait = b.image ? `<img class="portrait-img" src="${b.image}" alt="${b.name}" />` : `<div class="portrait-ph">${b.name.charAt(0)}</div>`;
    return `<p class="meta"><a href="#/drops">Drops</a> \u00b7 Act ${b.act}</p><header class="boss-head"><h1>${b.name}</h1><p class="lede">${b.title||""}</p></header><div class="boss-hero"><div class="portrait">${portrait}</div><div class="infobox"><h2>Quick info</h2><dl><dt>Location</dt><dd>${b.zone||"\u2014"}</dd><dt>Remnant</dt><dd>${b.remnant}</dd></dl></div></div><details class="ares" open><summary>Ares +0 baseline</summary>${dropTable(s0)}</details><details class="ares"><summary>Ares +3</summary>${dropTable(s3)}</details><details class="ares"><summary>Ares +5</summary>${dropTable(s5)}</details>`;
  }
  ...
  function skillsView(mid) {
    ...
    const byName = {}; actives.concat(passives).forEach((n) => { byName[n.name] = n; });
  ...
  function bindSkillPins() {
    ...
    if (!panel) return;
    app.querySelectorAll("button.sk").forEach((btn) => {
  ...
  document.querySelectorAll('header nav a[href="#/items"]').forEach((a) => {
    a.addEventListener("click", (e) => { e.preventDefault(); if (location.hash === "#/items" || location.hash === "#/items/all") render(); else location.hash = "#/items"; });
  });
